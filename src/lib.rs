...
mod tests {
    ...
    // Regression test for https://github.com/rbspy/proc-maps/issues/25: anonymous regions (those
    // `vmmap` reports with `SM=NUL`) must not be given the filename of a neighboring file-backed
    // region by `proc_regionfilename`.
    #[cfg(target_os = "macos")]
    #[test]
